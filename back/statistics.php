...
if ($request[action] == 'report')//---------------------------------- لیست آمار
{
	if($post[post])
	{
		...
		if (($data[limit] == "") OR (!$data[limit]))
			$data[limit] 	= $config[report][enrtyPerPage];
...
?>
	<tr>
		<td colspan="8">فروشی یافت نشد.</td>
	</tr>
<?
		}
?>
...
function get_cat_list($data, $parent = 0 )
{
	...
	if($data[$parent])
	{
		...
		{
			$count_query= "SELECT COUNT(*) FROM `card` WHERE `card_category` = '$v->category_id' AND `card_status` = '1' AND `card_show` = '1'";
			$count_card	= $db->fetch($count_query);
			$available	= $count_card['COUNT(*)'];
			
			$count_query= "SELECT COUNT(*) FROM `card` WHERE `card_category` = '$v->category_id' AND `card_status` = '2'";
			$count_card	= $db->fetch($count_query);
			$sold		= $count_card['COUNT(*)'];
			
			$count_query= "SELECT COUNT(*) FROM `card` WHERE `card_category` = '$v->category_id' AND `card_show` = '2'";
			$count_card	= $db->fetch($count_query);
			$invisiable	= $count_card['COUNT(*)'];
			...
			$html .= "$tab";
			$html .= '<tr><td class="first">'.$showPusher.' '.$v->category_title.'</td><td align="center">'.Convertnumber2farsi($available).'</td><td align="center">'.Convertnumber2farsi($sold).'</td><td align="center">'.Convertnumber2farsi($invisiable).'</td></tr>'."\n";
			$a--;
			if($child)
			{
				$i--;
